...
public class DatasessionsController {
	...
	private static final String[] columnHeaders = { "DataPoint", "DateTime", "P_Cryo_One", "P_Cryo_Two", "P_Chamber",
			"P_Turbo_Pump", "Gas_Flow_SCCM", "PinnaclePower", "Pinnacle_Power_Setpoint", "Pinnacle_Current",
			"Pinnacle_Voltage", "SSV_One_Power", "SSV_One_Power_Setpoint", "SSV_One_Current", "SSV_One_Voltage",
			"SSV_Two_Power", "SSV_Two_Power_Setpoint", "SSV_Two_Current", "SSV_Two_Voltage", "Conveyor_Speed_Combined",
			"Conveyor_Speed_Left", "Conveyor_Speed_Right", "Conveyor_Speed_Setpoint", "Conveyor_Engine_Left_Maximum",
			"Conveyor_Engine_Right_Maximum", "Conveyor_Position_Left", "Conveyor_Position_Right",
			"Conveyor_Position_Combined", "Conveyor_Mode", "SRM_Channel_2_Left", "SRM_Channel_3_Right" };
	...
	public void downloadSession(@PathVariable("id") Long id, HttpServletResponse response)
		...
		CSVWriter csvWriter = new CSVWriter(response.getWriter(), ';', CSVWriter.DEFAULT_QUOTE_CHARACTER,
				CSVWriter.DEFAULT_ESCAPE_CHARACTER, CSVWriter.DEFAULT_LINE_END);

		csvWriter.writeNext(columnHeaders);
		dataSessionsService.getDataPointsStreamFromSessionId(id).map(this::toCSVRow).forEach(csvWriter::writeNext);
	...
	public @ResponseBody byte[] getSessionAsCSV(@PathVariable("id") Long id)
		...
		StatefulBeanToCsv<DataPoint> beanToCsv = new StatefulBeanToCsvBuilder<DataPoint>(
				new BufferedWriter(new OutputStreamWriter(os))).withQuotechar(CSVWriter.NO_QUOTE_CHARACTER).build();
		beanToCsv.write(points);
